...
class DeviceJoggingControl(QtWidgets.QGroupBox):
    ...
    def joggingFastBackward(self):
        trace('[UI] jog fast backward clicked target=%r increment=%r', self.device.targetNow, self.incrementEdit.text())
    ...
    def joggingBackward(self):
        trace('[UI] jog backward clicked target=%r increment=%r', self.device.targetNow, self.incrementEdit.text())
        increment = self.incrementEdit.text()
        currenttarget = self.device.targetNow
        newTarget = float(currenttarget) - float(increment)
        self.device.sendTargetValue(str(newTarget))
    def joggingStop(self):
        trace('[UI] jog stop clicked controlType=%r', self.device.controlType)
    ...
    def joggingFordward(self):
        trace('[UI] jog forward clicked target=%r increment=%r', self.device.targetNow, self.incrementEdit.text())
        increment = self.incrementEdit.text()
        currenttarget = self.device.targetNow
        newTarget = float(currenttarget) + float(increment)
        self.device.sendTargetValue(str(newTarget))
    def joggingfastFordward(self):
        trace('[UI] jog fast forward clicked target=%r increment=%r', self.device.targetNow, self.incrementEdit.text())
        increment = self.incrementEdit.text()
        currenttarget = self.device.targetNow
        newTarget = float(currenttarget) + 2 * float(increment)
        self.device.sendTargetValue(str(newTarget))
