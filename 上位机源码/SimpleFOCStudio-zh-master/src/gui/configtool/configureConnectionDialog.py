...
class ConfigureSerailConnectionDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        trace('[UI] ConfigureSerailConnectionDialog.__init__ enter')

        self.setupUi(SimpleFOCDevice.getInstance())
        trace('[UI] ConfigureSerailConnectionDialog.__init__ done')

    def setupUi(self, device=None):
        trace('[UI] configure connection setupUi enter device=%r', device is not None)
        ...
        trace('[UI] configure connection setupUi done')

    def fillForm(self, deviceConnector):
        trace('[UI] configure connection fillForm port=%r rate=%r commandID=%r', deviceConnector.serialPortName, deviceConnector.serialRate, deviceConnector.devCommandID)
    ...
    def getConfigValues(self):
        trace('[UI] configure connection getConfigValues enter')
        values = {
        ...
        trace('[UI] configure connection getConfigValues result=%r', values)
        return values
