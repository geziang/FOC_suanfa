...
class PidGroupBox(QtWidgets.QGroupBox):
    ...
    def sendDerivativeGainAction(self):
        ...
        trace('[UI] PID D editingFinished pid=%r raw=%r', self.activePID.cmd, value)
        value = value.replace(',', '.')
    ...
    def sendProportionalGainAction(self):
        ...
        trace('[UI] PID P editingFinished pid=%r raw=%r', self.activePID.cmd, value)
        value = value.replace(',', '.')
    ...
    def sendIntegralGainAction(self):
        ...
        trace('[UI] PID I editingFinished pid=%r raw=%r', self.activePID.cmd, value)
        value = value.replace(',', '.')
    ...
    def sendRampAction(self):
        ...
        trace('[UI] PID ramp editingFinished pid=%r raw=%r', self.activePID.cmd, value)
        value = value.replace(',', '.')
    ...
    def sendLPFAction(self):
        ...
        trace('[UI] PID LPF editingFinished pid=%r raw=%r', self.activeLPF.cmd, value)
        value = value.replace(',', '.')
    ...
    def changePIDF(self):
        ...
        trace('[UI] PID selector changed index=%r', index)
        if index == 0:
            self.activePID = self.device.PIDVelocity
            self.activeLPF = self.device.LPFVelocity
        elif index == 1:
        ...
        trace('[UI] PID selector pull complete pid=%r', self.activePID.cmd)
        self.reloadPIDValues()
