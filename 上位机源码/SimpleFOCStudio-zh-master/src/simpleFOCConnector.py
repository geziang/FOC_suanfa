...
class SimpleFOCDevice:
    ...
    def __init__(self):
        ...
        if SimpleFOCDevice.__instance != None:
            ...
            trace('[DEVICE] singleton initialized; baud=%r', self.serialRate)


    def configureDevice(self, jsonValue):
        trace('[DEVICE] configureDevice enter; keys=%r', sorted(jsonValue.keys()))
    ...
    def configureConnection(self, configDict):
        trace('[DEVICE] configureConnection enter; config=%r', configDict)
        ...
        trace('[DEVICE] configureConnection done')
    ...
    def __initCommunications(self):
        trace('[SERIAL] opening port=%r rate=%r bytes=%r parity=%r stop=%r', self.serialPortName, self.serialRate, self.serialByteSize, self.serialParity, self.stopBits)
        ...
        trace('[SERIAL] port opened; receive thread started')

    def __closeCommunication(self):
        trace('[SERIAL] closing port')
        self.serialPort.close()
        trace('[SERIAL] port closed')

    def connect(self, connectionMode):
        trace('[DEVICE] connect enter; mode=%r connected=%r', connectionMode, self.isConnected)
        try:
            self.__initCommunications()
        except SerialException as serEx:
            trace_exception('open serial port', serEx)
    ...
    def disConnect(self):
        trace('[DEVICE] disConnect enter; connected=%r', self.isConnected)
        self.isConnected = False
        self.__closeCommunication()
        self.stateUpdater.stop()
        for listener in self.connectionStateListenerList:
            listener.connectionStateChanged(False)
        trace('[DEVICE] disConnect done')

    def addConnectionStateListener(self, listener):
        self.connectionStateListenerList.append(listener)

    def sendCommand(self, command):
        trace('[SERIAL TX] connected=%r command=%r', self.isConnected, command)
        if self.isConnected:
            self.serialPort.write((str(command) + '\n').encode('utf-8'))
            trace('[SERIAL TX] write complete bytes=%d', len((str(command) + '\n').encode('utf-8')))
        else:
            trace('[SERIAL TX] skipped because device is disconnected')

    def setCommand(self, command, value):
        trace('[SERIAL CMD] setCommand suffix=%r value=%r', command, value)
        if self.isConnected:
    ...
    def pullPIDConf(self, pid, lpf):
        trace('[PULL] pullPIDConf enter pid=%r lpf=%r', pid.cmd, lpf.cmd)
        ...
        trace('[PULL] pullPIDConf done pid=%r', pid.cmd)

    def pullConfiguration(self):
        trace('[PULL] pullConfiguration enter')
        time.sleep(5 / 1000)
        ...
        trace('[PULL] pullConfiguration done')
...
class SerialPortReceiveHandler(QtCore.QThread):
    ...
    def handle_received_data(self, data):
        ...
        if self.isDataReceivedMonitoring(data):
            try:
                ...
                trace_exception('parse monitoring data ValueError', error)
                logging.error(error, exc_info=True)
                logging.error('data =' + str(data), exc_info=True)
            except IndexError as error:
                trace_exception('parse monitoring data IndexError', error)
                logging.error(error, exc_info=True)
                logging.error('data =' + str(data), exc_info=True)
    ...
    def run(self):
        trace('[SERIAL RX THREAD] started')
        try:
            ...
            trace_exception('serial receive thread', serialException)
            logging.error(serialException, exc_info=True)
        except TypeError as typeError:
            trace_exception('serial receive thread TypeError', typeError)
            logging.error(typeError, exc_info=True)
        except AttributeError as ae:
            trace_exception('serial receive thread AttributeError', ae)
            logging.error(ae, exc_info=True)            

    def stop(self):
        trace('[SERIAL RX THREAD] stop requested')
